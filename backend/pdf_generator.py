...
class PDFReportGenerator:
    ...
    def generate_investment_report(self, report_data: Dict[str, Any]) -> str:
        ...
        if 'historical_data' in report_data and report_data['historical_data']:
            chart_path = self._generate_price_chart(report_data['historical_data'], ticker)
            if chart_path:
                story.append(Paragraph("Price Performance Chart", self.styles['SectionHeader']))
                # Note: In a full implementation, you'd embed the chart image here
                story.append(Paragraph(f"Chart saved to: {chart_path}", self.styles['Normal']))
                story.append(Spacer(1, 20))
